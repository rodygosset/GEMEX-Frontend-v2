...
const SearchResultCard = ({ data, itemType, href, globalMetaData, isSelected, areLinksDisabled, onClick }: Props) => {
	...
	const getItemTitle = () => {
		if ("username" in data && "nom" in data && "prenom" in data) {
	...
	return (
		<li
			...
			className={cn(
				...
				"w-full flex items-center flex-wrap px-[32px] py-4",
				isSelected ? "bg-blue-600/10 border-2 border-blue-600" : "hover:bg-blue-600/5",
				"max-md:flex-col max-md:items-start gap-4"
			)}>
				...
				{searchResultConfig.map((field, index) => {
					...
					return (
						<li
							...
							<span className="text-blue-600/80">{getFilterLabel(field, fieldConf)}</span>
							{
								...
								fieldConf.type in apiURLs ? (
									...
									<span className={cn("text-blue-600 flex-1 whitespace-nowrap overflow-hidden text-ellipsis")}>{metaData[index]}</span>
								)
							}
						</li>
					)

...
const OperationReportCard = ({ fiche }: Props) => {
	...
	const getFicheUser = async () => {
		...
		return await makeAPIRequest<typeof user, typeof user>(session, "get", "users", `id/${fiche.auteur_id}`, undefined, (res) => res.data)
	}
	...
	const getFicheUserRole = async (role_id: number) => {
		...
		return await makeAPIRequest<UserRole, typeof userRole>(session, "get", "roles", `id/${role_id}`, undefined, (res) => res.data.titre)
	}
	...
	return item && user ? (
		<article
			className={cn(
				"w-[320px]",
				"flex flex-col gap-4 p-[32px] rounded-[8px] ",
			...
			<div className="w-full flex items-center gap-4">
				<div className="w-[192px] flex flex-col">
					...
					<span className={cn("text-sm font-normal text-purple-500", "whitespace-nowrap overflow-hidden text-ellipsis")}>{item.nom}</span>
				</div>
				<Link
		...
		<Skeleton className="w-[320px] h-[170px] rounded-[8px] flex flex-col gap-4 p-[32px] opacity-80">

...
const ExpositionListItem = ({ cycle, expositionId, onRefresh }: Props) => {
	// state
	...
	const getCycleExposition = async () => {
		...
		const nomExposition = await makeAPIRequest<Exposition, string>(session, "get", "expositions", `id/${expositionId}`, null, (res) => res.data.nom)
		...
		if (latestEvaluation && !(latestEvaluation instanceof Error)) {
			...
			// get element ids for the exposition

			const ids = await makeAPIRequest<Element[], number[]>(session, "post", "elements", "search", { exposition_id: expositionId }, (res) =>
				res.data.map((element) => element.id)
			)

			if (ids && !(ids instanceof Error)) {
				...
				const note = await makeAPIRequest<MoisCycle, number | undefined>(
					...
					undefined,
					(res) => {
						const moisCycle = res.data
						const evaluations = moisCycle.evaluations.filter(
							(evaluation) => ids.includes(evaluation.element_id) && typeof evaluation.note == "number"
						)
						if (evaluations.length == 0) return 0
						return evaluations.reduce((acc, curr) => acc + (curr.note ?? 0), 0) / evaluations.length
					}
				)
	...
	const handleDelete = async () => {
		...
		await makeAPIRequest(session, "put", "cycles", `id/${cycle.id}`, {
				...
				<span className="w-full whitespace-normal text-base font-semibold text-blue-600 flex flex-row gap-2 items-center">
					...
					{cycleExposition.note ? (
						<>
							<FontAwesomeIcon
								icon={faCircle}
								className="text-blue-600/40 text-[0.4rem] mx-2"
							/>
							<b
								className={`text-base font-bold ${
									cycleExposition.note >= 16 ? "text-success" : cycleExposition.note >= 15 ? "text-warning" : "text-error"
								}`}>
								{cycleExposition.note.toFixed(2)}
							</b>
						</>
					) : (
						<></>
					...
					{cycleExposition.latestEvaluation ? (
						...
						cycleExposition.latestEvaluation?.date_rendu_reelle ? (
							<span className="text-sm font-normal text-blue-600/60">
								{getMonthAndYear(new Date(cycleExposition.latestEvaluation.date_rendu_reelle))}
							</span>
						) : (
							// if it's planned
							<span className="text-sm font-bold text-blue-600/60">
								Prévu pour {getMonthAndYear(new Date(cycleExposition.latestEvaluation.date_rendu))}
							</span>
						)
					) : (
						<>

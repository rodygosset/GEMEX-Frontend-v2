...
const CurrentMonthlyAssessmentWidget = ({ moisCycle, cycleStartDate }: Props) => {
	// utils
			...
			{moisCycle ? (
				<div className="min-h-[210px] h-full w-full flex flex-col items-center justify-center">
					<div className="flex flex-row items-center justify-center w-full gap-16">
						<div
							className={`h-[1px] flex-1 ${(moisCycle.note || 0) >= 16 ? "bg-success/20" : ""} ${
								(moisCycle.note || 0) >= 15 && (moisCycle.note || 0) < 16 ? "bg-warning/20" : ""
							} ${(moisCycle.note || 0) < 15 ? "bg-error/20" : ""}`}></div>
						<span
							className={`text-[12rem] ${(moisCycle.note || 0) >= 16 ? "text-success" : ""} ${
								(moisCycle.note || 0) >= 15 && (moisCycle.note || 0) < 16 ? "text-warning" : ""
							} ${(moisCycle.note || 0) < 15 ? "text-error" : ""}`}>
					...
					<p
						className={`text-base font-normal text-center w-fit ${(moisCycle.note || 0) >= 16 ? "text-success/60" : ""} ${
							(moisCycle.note || 0) >= 15 && (moisCycle.note || 0) < 16 ? "text-warning/60" : ""
						} ${(moisCycle.note || 0) < 15 ? "text-error/60" : ""}`}>
						Moyenne du mois
					</p>
				</div>
			) : (
				...
				<div className="h-full w-full flex flex-col items-center justify-center gap-4">
					...
					<p className="text-base font-normal text-blue-600/60">Aucune évaluation en cours</p>
				</div>
			)}
		</DashboardWidget>
	)

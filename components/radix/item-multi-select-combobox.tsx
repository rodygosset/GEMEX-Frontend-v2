...
const ItemMultiSelectCombobox = (
    ...
    useEffect(() => {
        ...
        onSelect(selected.filter(item => options.find(option => option.value == item)).map(item => options.find(option => option.value == item) as SelectOption))
    ...
    return (
        <MultiSelectCombobox
            className={className}
            options={options}
            selected={isLoading ? [] : selected?.map(item => options.find(o => o.value == item) ?? { value: item, label: "Item non existant" })}
            onSelect={onSelect}
        />
    )

...
const ResultsStep = ({ report }: Props) => {
	// state
			...
			<section className={styles.container}>
				<div className={styles.content}>
					<div className={styles.header}>
						<h3>Résultats</h3>
						<p>
							Pour la période allant du <strong>{toLocaleDateString(report.date_debut)}</strong> au{" "}
							<strong>{toLocaleDateString(report.date_fin)}</strong>
						</p>
					</div>
					<ul className={styles.groupResults}>
						{report.groupes_expositions.map((group, index) => (
							<li key={`groupe-${group.nom}-${index}`}>
								<h4>{group.nom}</h4>
								<p className={getRatioClassNames(100 - group.taux)}>{(100 - group.taux).toFixed(2)}%</p>
							</li>
						))}
					</ul>
					<div className={styles.chartContainer}>
						<div className="relative w-full min-[992px]:w-[600px] min-[1024px]:w-full h-[400px] min-h-[400px] max-[992px]:h-[400px] max-[992px]:min-h-[400px]">
							<BarChart
					...
					{report.groupes_expositions.length > 0 && selectedGroup ? (
						<>
							<div className={styles.chartContainer}>
								<div className="relative w-full min-[992px]:w-[600px] min-[1024px]:w-full h-[400px] min-h-[400px]">
									<BarChart
							...
							<div className={styles.selectSectionHeader}>
								...
								<Select
									name="expositions"
									options={
										getSelectedGroup()?.expositions.map((expo) => ({
											label: expo.nom,

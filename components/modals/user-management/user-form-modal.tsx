...
const UserFormModal = ({ isVisible, closeModal, data, refresh }: Props) => {
	// form data
	...
	useEffect(() => {
		...
		makeAPIRequest<UserRole[], void>(session, "get", "roles", undefined, undefined, (res) => {
			setRoleOptions(
				res.data
					.map((role) => ({
	...
	useEffect(() => {
		...
		makeAPIRequest<UserRole, void>(session, "get", "roles", `id/${roleId}`, undefined, (res) => setSelectedRole(res.data))
	}, [roleId])
	...
	const postNewUser = () => {
		...
		return makeAPIRequest<User, void>(session, "post", "users", undefined, newUser)
	}
	...
	const updateUser = () => {
		...
		return makeAPIRequest<User, void>(session, "put", "users", data.username, updatedUser)
	}
	...
	const validateForm = async () => {
		...
		if (!newErrorFields.includes("username")) {
			...
			// @ts-ignore
			if ((user && !data) || (data && user && !(user instanceof Error) && data.username !== user.username)) {
				newErrorFields.push("username")
				validated = false

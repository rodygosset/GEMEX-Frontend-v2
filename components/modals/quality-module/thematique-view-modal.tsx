...
const ThematiqueViewModal = ({ thematique, isOpen, onClose, onEdit, onDelete }: Props) => {
	// handlers
	...
	return (
		<ModalContainer isVisible={isOpen}>
			<section className="flex flex-col gap-8 min-w-[400px] max-w-3xl overflow-auto bg-white rounded-2xl p-[32px]">
				{thematique ? (
						...
						<div className="w-full flex flex-row gap-4 items-start">
							<div className="flex-1 flex flex-col gap-2">
								...
								<p className="flex flex-col">
									...
									<span className="text-sm font-normal text-purple-600/80">Pondération à {thematique.ponderateur}</span>
								</p>
							</div>
							<ContextMenu
								options={[
										...
										<Fragment key={question.id}>
											<li className="flex flex-col">
												<p className="flex flex-row items-center">
													{question.titre ? <span className="text-sm font-normal text-blue-600/60">{question.titre}</span> : <></>}
													{question.optional ? (
														<>
															<FontAwesomeIcon
																icon={faCircle}
																className="text-blue-600/60 mx-2 text-[0.3rem]"
															/>
													...
													{question.grille ? (
														<>
															<FontAwesomeIcon
																...
																className="text-blue-600/60 mx-2 text-[0.3rem]"
															/>
															<span className="text-sm font-normal text-blue-600/60">Grille</span>
														</>
													) : (
														<></>
													)}
												</p>
												<span className="text-sm font-normal text-blue-600/80">{question.description}</span>
												<p className="text-base font-normal text-blue-600">{question.question}</p>
											</li>
											{index !== thematique.questions.length - 1 ? <div className="w-full h-[1px] bg-blue-600/20 my-2"></div> : <></>}
										</Fragment>
									))}
								</ul>
							) : (

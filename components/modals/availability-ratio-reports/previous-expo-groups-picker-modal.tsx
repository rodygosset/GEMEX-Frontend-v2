...
const PreviousExpoGroupsPickerModal = ({ isVisible, closeModal, expoGroups, onChange }: Props) => {
	// state
	...
	useEffect(() => {
		...
		makeAPIRequest<RapportTauxDisponibilite[], void>(session, "get", "rapports", "?max=3", undefined, (res) => setReports(res.data.reverse()))
	}, [session])
					...
					<ul className={styles.reportList}>
						{reports.map((report, index) => (
							<li
								...
								<h3>
									Rapport allant du {toDateString(report.date_debut)} au {toDateString(report.date_fin)}
								</h3>

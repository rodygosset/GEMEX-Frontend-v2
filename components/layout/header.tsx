...
const Header = () => {
	...
	const hiddenHeaderRoutes = ["/login", "/404", "/401"]

	const { navHistory } = useContext(Context)

	const shouldShowHeader = () => !hiddenHeaderRoutes.includes(router.pathname) && navHistory[navHistory.length - 1] != "/401"
	...
	const shouldShowQualityLink = () => userRole && userRole.permissions.includes("qualite")

	const shouldShowManageLink = () => userRole && userRole.permissions.includes("manage")
	...
	return shouldShowHeader() ? (
		<>
			<header className={cn(styles.header, "border-b border-blue-600/10", "bg-neutral-50/40 backdrop-blur-3xl")}>
				<Link
					href="/"

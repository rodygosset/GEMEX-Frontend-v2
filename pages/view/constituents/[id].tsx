import { Constituent } from "@conf/api/data-types/constituent"
...
// this page displays information about a given Constituent object
// the data is retrieved in getServerSideProps

const itemType = "constituents"

interface Props {
    data: Constituent | null;
    extra: {
        element: string;
        article: string;
    } | null;
...
const ViewConstituent: NextPage<Props> = (
    {
        data,
        extra
    }
    ...
    return (
        ...
        <>
            <Head>
				<title>{data.nom} (Constituent)</title>
				<meta name="description" content={`Informations sur le constituent ${data.nom}`} />
			</Head>
            <ViewTemplate
                itemType={itemType}
                itemTitle={data.nom}
...
export const getServerSideProps: GetServerSideProps<Props> = async (context) => {
    ...
    const constituentId = context.query.id
    ...
    const data = await SSRmakeAPIRequest<Constituent, Constituent>({
        session: session,
        verb: "get",
        itemType: itemType,
        additionalPath: `id/${constituentId}`, 
    ...
    // in case the provided constituent_id doesn't exist in the database

    else if(is404) return { notFound: true } // show the 404 page

    // retrieving the extra data we need to display

    const element = await getExtraSSRData(
        session, 
        "elements", 
        (data as Constituent).element_id
    ) 

    const article = await getExtraSSRData(
        session, 
        "articles", 
        (data as Constituent).article_id
    ) 
    ...
    return {
        props: {
            ...
            extra: {
                ...
                article: article ? article : "Erreur",
            }
        }
    }
...
export default ViewConstituent
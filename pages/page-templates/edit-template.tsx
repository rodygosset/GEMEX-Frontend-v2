...
const EditTemplate = ({ itemType, excluded, defaultValues }: Props) => {
	// access url query
	...
	const buildSubmitData = () => {
		...
		for (const field in formData) {
			...
			if (itemType.includes("fiches") && formData[field].conf.type == "users" && !userRole?.permissions.includes("manage")) {
				continue
			}
					...
					<div className="flex flex-1 flex-col min-w-[350px] gap-4">
						<Input
							className={cn(
								...
								itemType == "fiches" && getFicheType() == "relance"
									? "text-yellow-600 border-yellow-600/20 placeholder:text-yellow-600/60"
									: "",
								itemType == "fiches" && getFicheType() == "panne" ? "text-red-600 border-red-600/20 placeholder:text-red-600/60" : "",
								itemType == "fiches" && getFicheType() == "systématique"

...
const Search: NextPage<Props> = ({ initSearchParams, initSearchResults }: Props) => {
	// search form
	...
	const searchParams = useWatch({ control: searchForm.control })
		...
		<main className="flex flex-col px-[7%] gap-y-16 pt-6">
			<div className="w-full flex flex-row items-center gap-x-16 gap-y-[32px] flex-wrap">
				...
				<div className="flex flex-col flex-1 min-w-[350px]">
					...
					<p className="text-base text-blue-600 text-opacity-40">
						Passer en revue les évaluations passées en fonction des thématiques et des expositions
					</p>
				</div>
				{csv ? (
					<Link
									...
									<FormControl>
										<ItemMultiSelectCombobox
											...
											onSelect={(options) =>
												searchForm.setValue(
													field.name,
													options.map((o) => o.value as number)
												)
											}
										/>
									</FormControl>
									<FormDescription>Sélectionnez les thématiques pour lesquelles vous souhaitez consulter les évaluations</FormDescription>
								</FormItem>
							)}
						/>
						<FormField
							...
							render={({ field }) => (
								<FormItem className="flex-1 min-w-[250px]">
									...
									<FormDescription>Sélectionnez les expositions pour lesquelles vous souhaitez consulter les évaluations</FormDescription>
								</FormItem>
							)}
						/>
					</form>
...
export const getServerSideProps: GetServerSideProps<Props> = async (context) => {
	...
	const initSearchResults =
		(await SSRmakeAPIRequest<Evaluation[], SearchResultsByMonth[]>({
			session,
			verb: "post",
